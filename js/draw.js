/**
 * Canvas drawing utilities for annotation overlays.
 *
 * Ported verbatim (semantics-preserving) from the SvelteKit frontend's
 * components/web/frontend/src/lib/draw.ts so the standalone demo renders
 * identically to the full app. Plain ES module, no build step.
 */
...
export function annotationFrameStep(annotations) {
 ...
 * Mirrors the app: walk backward up to 8 frames so duplicate-frame source
 * video (e.g. 30fps encoded at 60) doesn't flicker.
 */
export function resolveFrame(annotations, frame) {
...
export function effectiveId(p) {
	return p.player_id;
}

export function playerColor(p, colorMode, alpha = false) {
	const id = effectiveId(p);
	if (id < 0) {
...
function drawSkeleton(ctx, p, sx, sy) {
 ...
 * sx/sy scale from annotation coordinate space to canvas pixels.
 */
export function drawAnnotations(ctx, fd, toggles, sx, sy) {
	// 1. Segmentation masks
	if (toggles.masks) {
		for (const p of fd.players) {
			if (!p.mask_polygon) continue;
			if (effectiveId(p) < 0 && toggles.colorMode !== 'id') continue;
			ctx.save();
			if (p.bbox) {
