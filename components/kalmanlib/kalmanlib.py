# https://github.com/XuMuK1/psmo/blob/2025/seminars/sem_KF/kalmanlib.py
import numpy as np
import scipy as sp
...
class KalmanFilter:
    ...
    def filterSignal(self, signal, returnK=False):
        ...
        for i in np.arange(0, signal.shape[1]):
    ...
    def smoothSignal(self, filteredSignal, errs, aprSignal, aprErrs, K=None, estLag1=False, returnGains=False):
        ...
        for t in np.arange(errs.shape[-1] - 2, -1, -1):
            ...
            smoothedSignal[..., t] = filteredSignal[..., t] + smoothingGain @ (
                smoothedSignal[..., t + 1] - aprSignal[..., t + 1]
            )
            smoothedErrs[..., t] = errs[..., t] + smoothingGain @ (
                smoothedErrs[..., t + 1] - aprErrs[..., t + 1]
            ) @ np.transpose(smoothingGain)
            if estLag1:
                # if(t<errs.shape[-1]-2):
                lag1[..., t] = (
                    smoothingGain @ smoothedErrs[..., t + 1]
                )  # errs[...,t]@smoothingGain.T + gains[-2]@(lag1[...,t+1] - self.A@errs[...,t+1])@smoothingGain.T
    ...
    def fit(
        ...
        Args:
            ...
            fixA (bool, optional): Whether to fit A. Defaults to False
            fixB (bool, optional): Whether to fit B, Defaults to False
            fixRx (bool, optional): Whether to fit Rxю Defaults to False
            fixRy (bool, optional): Whether to fit Ryю Defaults to False
            fixStartMean (bool, optional): Whether to fit startMeanю Defaults to False
            fixStartCov (bool, optional): Whether to fit startCov. Defaults to False
            verbose (bool, optional): Whether to print tech info. Defaults to False
        """
        if verbose:
            techStr = "[LOG][KalmanFilter|fit]"

        for i in tqdm(np.arange(Niter)):
            filteredSignal, errs, aprSignal, aprErrs, K = self.filterSignal(signal, returnK=True)
            smoothedSignal, smoothedErrs, smoothGains, lag1 = self.smoothSignal(
                filteredSignal, errs, aprSignal, aprErrs, K=K, estLag1=True, returnGains=True
            )

            XY = np.einsum("it,jt -> ij", signal, smoothedSignal)
            YY = np.einsum("it,jt -> ij", smoothedSignal, smoothedSignal) + np.sum(smoothedErrs, axis=-1)
            XX = np.einsum("it,jt -> ij", signal, signal)
            Yp1Y = np.einsum("it,jt -> ij", smoothedSignal[..., 1:], smoothedSignal[..., :-1]) + np.sum(lag1, axis=-1)
            YYnolast = YY - smoothedSignal[:, None, -1] * smoothedSignal[None, :, -1] - smoothedErrs[..., -1]
            YYnofirst = YY - smoothedSignal[:, None, 0] * smoothedSignal[None, :, 0] - smoothedErrs[..., 0]
            ...
            if not fixB:
                self.B = XY @ np.linalg.inv(YY)

            if not fixRx:
                self.Rx = (XX - self.B @ XY.T - XY @ self.B.T + self.B @ YY @ self.B.T) / (signal.shape[-1])

            if not fixA:
                self.A = Yp1Y @ np.linalg.inv(YYnolast)

            if not fixRy:
                self.Ry = (YYnofirst - self.A @ Yp1Y.T - Yp1Y @ self.A.T + self.A @ YYnolast @ self.A.T) / (
                    signal.shape[-1] - 1
                )
            ...
            if verbose:
                # debug place
                if i % 50 == 0:
                    print(techStr + "mean gain:", np.mean(smoothGains, axis=-1))
                    print(techStr + "mean smoothed signal:", np.mean(smoothGains, axis=-1))
                    print(techStr + "A:", self.A)
                    print(techStr + "B:", self.B)
                    print(techStr + "Rx:", self.Rx)
                    print(techStr + "Ry:", self.Ry)
                    print(techStr + "startMean:", self.startMean)
                    print(techStr + "startCov:", self.startCov)
