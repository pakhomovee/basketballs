...
def _gt_to_player_detections(
    ...
    for frame_id, dets in gt.items():
        detections[frame_id] = [
            Player(bbox=[int(v) for v in d["bbox"]], player_id=-1)
            for d in dets
        ]
    return detections
...
def _detections_to_pred(
    ...
    for frame_id, players in detections.items():
        dets = [
            {"id": p.player_id, "bbox": list(p.bbox)}
            for p in players
            if p.player_id >= 0
        ]
        if dets:
...
def run_benchmark(
    ...
    *write_visuals* is False.
    ...
    agg = {
        "TP": 0, "FP": 0, "FN": 0, "IDSW": 0, "total_gt": 0, "sum_iou": 0.0,
    }
...
def _print_metrics(name: str, m: dict) -> None:
    ...
    print(f"  TP: {m.get('TP', 0)}  FP: {m.get('FP', 0)}  FN: {m.get('FN', 0)}  IDSW: {m.get('IDSW', 0)}  GT: {m.get('total_gt', 0)}")
    print(f"{'═' * 50}")
...
def main():
    ...
    parser.add_argument(
        "data_dir",
        help="Root directory (with video/ and annotation/ subdirs)",
    )
    parser.add_argument(
