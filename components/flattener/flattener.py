...
def _meters_to_norm(
    x_m: float, y_m: float, length_m: float, width_m: float
) -> Tuple[float, float]:
...
def _norm_to_pixel(
    x: float, y: float, width: int, height: int, padding: int = _COURT_PADDING
) -> Tuple[int, int]:
...
if __name__ == "__main__":
    ...
    _team1 = np.array([[_COURT_METERS[CourtType.NBA][0] / 2, _COURT_METERS[CourtType.NBA][1] / 2], [-10.0, 2.0], [-10.0, -2.0]])   # слева
