"""
Court detection visualization: CLI for validating court keypoint predictions.

Visualizes GT vs predicted court points on validation images and video.
Uses court_detector datasets and model.
"""
...
COURT_DETECTOR_DIR = Path(__file__).parent.parent / "court_detector"
DETECTION_CONF = 0.25
...
def main():
    ...
    parser.add_argument(
        "--data-root",
        type=Path,
        default=COURT_DETECTOR_DIR / "yolo_combined_data",
...
def run_deepsportradar(args):
    ds_root = COURT_DETECTOR_DIR / "dataset" / "deepsportradar_instants_dataset"
    ...
    def show():
        ...
        for x, y, t in FIBA_COURT_POINTS:
            pts.append([(x + 14.0) * 100, (-y + 7.5) * 100, 0.0])
            types.append(t)
...
def run_sportcenter(args):
    ds_root = COURT_DETECTOR_DIR / "dataset" / "sportcenter_camerapose_dataset"
...
def run_roboflow(args):
    ds_root = COURT_DETECTOR_DIR / "dataset" / "roboflow_court_detection"
