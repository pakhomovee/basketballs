...
class CourtDetector:
    ...
    def predict_keypoints(self, frame_rgb) -> (np.ndarray, np.ndarray, np.ndarray):
        ...
        # Take the first (and only) detected court instance
        kpts_xy = preds.keypoints.xy[0].cpu().numpy()      # (num_kpts, 2)
        kpts_conf = preds.keypoints.conf[0].cpu().numpy()   # (num_kpts,)

        # Keep only keypoints with confidence above threshold
        visible = kpts_conf > self.conf
        pred_centers = kpts_xy[visible]
        pred_cls = np.where(visible)[0].astype(int)
        pred_confs = kpts_conf[visible]
...
def main():
    detector = CourtDetector(model_pretrained="yolo26s-pose.yaml")
    detector.train(epochs=5, lr0=0.01, lrf=0.001, need_prepare_dataset=True, batch=4)
