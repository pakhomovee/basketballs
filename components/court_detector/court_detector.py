...
class CourtDetector:
    def __init__(self, model_path: str | Path = Path(__file__).parent.parent.parent / "model" / "court_detection_model.pt"):
        self.model = YOLO(model_path)
        self.model_path = model_path
        self.conf = 0.3

    def train(self, dataset_path: Path = Path(__file__).parent / "yolo_combined_data", 
                                need_prepare_dataset: bool = False,
                                epochs: int = 30, batch: int = 16, imgsz: int = 640, 
                                save = True, 
                                lr0=0.01, lrf=0.001):
    ...
    def predict_keypoints(self, frame_rgb) -> (np.ndarray, np.ndarray):
        ...
        num_keypoints = pred_centers.shape[0]
        return pred_centers, pred_cls

    def predict_court_homography(self, frame_rgb: np.ndarray, court_points: list[tuple] = NBA_COURT_POINTS) -> (np.ndarray, np.ndarray, Optional[np.ndarray]):
